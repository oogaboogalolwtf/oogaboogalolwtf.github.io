((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.clients.dart.js_4",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={hS:function hS(d,e){this.a=d
this.b=e},
Hv(d){return new C.dU("rem",d*0.0625)}}
A=c[0]
C=c[27]
B=a.updateHolder(c[14],B)
B.hS.prototype={
gfd(){var x=this.b,w=y.c
w=A.F(w,w)
if(x!=null)w.i(0,"top",C.cd(x.b)+x.a)
x=this.a
if(x!=null)w.i(0,"left",C.cd(x.b)+x.a)
return w},
$ix2:1}
var z=a.updateTypes([]);(function inheritance(){var x=a.inherit
x(B.hS,A.m)})()
A.bV(b.typeUniverse,JSON.parse('{"hS":{"x2":[]}}'))
var y={c:A.u("i")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.clients.dart.js_4",e:"endPart",h:b})})($__dart_deferred_initializers__,"fKvRCTeZSu/K9muO7OTl1u+t9HA=");
//# sourceMappingURL=main.clients.dart.js_4.part.js.map
