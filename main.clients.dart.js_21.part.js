((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.clients.dart.js_21",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,K,I,D,E,B={mA:function mA(d,e,f,g,h){var _=this
_.c=d
_.d=e
_.e=f
_.f=g
_.a=h},vd:function vd(){},lE:function lE(d,e){this.c=d
this.a=e},pu:function pu(d){this.a=d},ps:function ps(d){this.a=d},pt:function pt(d){this.a=d},
eA(d){var x=$.yl.k(0,d)
if(x==null){x=new B.lm(d,A.b([],y.A))
$.yl.i(0,d,x)}return x},
lX:function lX(d,e){this.c=d
this.a=e},
ln:function ln(d,e){this.a=d
this.b=e},
ii:function ii(d,e,f,g,h){var _=this
_.e=d
_.f=e
_.b=f
_.c=g
_.a=h},
nQ:function nQ(d,e,f,g,h,i){var _=this
_.d$=d
_.e$=e
_.dx=null
_.dy=f
_.b=_.a=null
_.c=g
_.d=null
_.e=h
_.r=_.f=null
_.w=i
_.z=_.y=_.x=null
_.Q=!1
_.as=!0
_.at=!1
_.cy=_.cx=_.CW=_.ch=_.ay=null
_.db=!1},
d4:function d4(d,e,f,g){var _=this
_.e=d
_.f=e
_.r=null
_.w=f
_.a=null
_.b=g
_.d=_.c=null},
lm:function lm(d,e){var _=this
_.a=d
_.e=_.d=_.c=_.b=$
_.f=e
_.r=!0},
oY:function oY(d){this.a=d},
oZ:function oZ(){},
H1(d){return new B.lE(A.bE(d.k(0,"id")),null)}},F,N,L,H,M,G
A=c[0]
C=c[2]
K=c[20]
I=c[24]
D=c[27]
E=c[21]
B=a.updateHolder(c[6],B)
F=c[32]
N=c[11]
L=c[31]
H=c[17]
M=c[12]
G=c[25]
B.mA.prototype={
ag(d){return new A.X(this.rd(d),y.d)},
rd(d){var x=this
return function(){var w=d
var v=0,u=1,t=[],s
return function $async$ag(e,f,g){if(f===1){t.push(g)
v=u}while(true)switch(v){case 0:s=y.i
v=2
return e.b=G.af(A.b([I.ou(null,null,x.c,null),E.b3(A.b([new D.L(x.d,null)],s),"member-card-name",null,null),E.b3(A.b([new D.L("As "+x.e,null)],s),"member-card-role",null,null),G.af(A.b([new D.L(x.f,null)],s),null,"member-card-footer",null,null,null,null)],s),null,"member-card",null,null,null,null),1
case 2:return 0
case 1:return e.c=t.at(-1),3}}}}}
B.lE.prototype={
ag(d){return new A.X(this.r5(d),y.d)},
r5(d){var x=this
return function(){var w=d
var v=0,u=2,t=[],s
return function $async$ag(e,f,g){if(f===1){t.push(g)
v=u}while(true)switch(v){case 0:v=x.c==null?3:4
break
case 3:s=y.i
v=5
return e.b=E.eu(A.b([new I.aE(!1,"/images/heart.svg",null,null),G.af(A.b([E.et(L.bU,null),E.c5(L.c6,null)],s),null,"error-div",null,null,null,null)],s),null,"crew-view-main"),1
case 5:v=1
break
case 4:s=A.b([new I.aE(!1,"/images/heart.svg",null,null)],y.i)
s.push(K.rU(new K.eC(new B.pu(x),null)))
v=6
return e.b=E.eu(s,null,"crew-view-main"),1
case 6:case 1:return 0
case 2:return e.c=t.at(-1),3}}}},
o6(d){var x,w,v,u,t=null
y.B.a(d)
x=A.b([],y.i)
for(w=d.length,v=0;v<d.length;d.length===w||(0,A.ad)(d),++v){u=d[v]
x.push(new B.mA(H.xR(u.c),u.a,u.b,"",t))}return G.af(x,t,t,t,"cards",t,t)}}
B.lX.prototype={
ag(d){return new A.X(this.ra(d),y.d)},
ra(d){var x=this
return function(){var w=d
var v=0,u=1,t=[],s
return function $async$ag(e,f,g){if(f===1){t.push(g)
v=u}while(true)switch(v){case 0:s=A.b([],y.i)
s.push(new D.aH("title",null,null,null,null,null,new D.L(x.c,null),null,null))
v=2
return e.b=new B.ii(F.cA,null,null,s,null),1
case 2:return 0
case 1:return e.c=t.at(-1),3}}}}}
B.ln.prototype={
a8(){return"AttachTarget."+this.b}}
B.ii.prototype={
bg(){var x=A.co(y.h),w=($.aU+1)%16777215
$.aU=w
return new B.nQ(null,!1,x,w,this,C.B)}}
B.nQ.prototype={
ls(){var x,w,v=this.e
v.toString
x=y.U.a(v).e
v=this.d
v.toString
w=y.O
w=new B.d4(A.b([],w),x,v,A.b([],w))
w.a=y.m.a(new b.G.Text(""))
v=B.eA(x)
C.b.A(v.f,w)
v.r=!0
return w},
d_(){var x,w=this.e
w.toString
y.U.a(w)
x=this.d$
x.toString
y.j.a(x)
x.stW(w.e)
x.slh(w.f)},
es(){var x,w
this.jm()
x=this.d$
x.toString
y.j.a(x)
w=this.d
w.toString
x.srL(w)},
dd(){var x,w
this.n0()
x=this.d$
x.toString
y.j.a(x)
w=B.eA(x.f)
C.b.ae(w.f,x)
w.f1()}}
B.d4.prototype={
stW(d){var x=this,w=x.f
if(w===d)return
w=B.eA(w)
C.b.ae(w.f,x)
w.f1()
x.f=d
w=B.eA(d)
C.b.A(w.f,x)
w.r=!0
B.eA(x.f).f1()},
slh(d){return},
srL(d){if(this.w===d)return
this.w=d
B.eA(this.f).ma(!0)},
fO(d,e){var x,w,v,u,t=this
try{x=d.a
if(x==null)return
w=e==null?null:e.a
if(w==null&&C.b.a5(t.e,x))return
if(w!=null&&!C.b.a5(t.e,w))w=null
v=t.e
C.b.ae(v,x)
u=w!=null?C.b.cI(v,w)+1:0
C.b.fV(v,u,x)
B.eA(t.f).f1()}finally{d.fQ()}},
ae(d,e){this.mI(0,e)
C.b.ae(this.e,e.a)
B.eA(this.f).f1()}}
B.lm.prototype={
geE(){var x,w=this,v=w.b
if(v===$){x=y.z.a(y.m.a(b.G.document).querySelector(w.a.b))
x.toString
w.b!==$&&A.d1()
w.b=x
v=x}return v},
gex(){var x,w=this,v=w.d
if(v===$){x=new B.oY(w).$0()
w.d!==$&&A.d1()
w.d=x
v=x}return v},
glP(){return new A.X(this.tk(),y.F)},
tk(){var x=this
return function(){var w=0,v=1,u=[],t,s
return function $async$glP(d,e,f){if(e===1){u.push(f)
w=v}while(true)switch(w){case 0:t=y.z
s=t.a(x.gex().a.nextSibling)
case 2:if(!!0){w=3
break}if(!(s!=null&&s!==x.gex().b)){w=3
break}w=4
return d.b=s,1
case 4:s=t.a(s.nextSibling)
w=2
break
case 3:return 0
case 1:return d.c=u.at(-1),3}}}},
glG(){var x,w,v,u,t,s=this,r=s.e
if(r===$){x=A.F(y.N,y.m)
for(w=s.glP(),v=w.$ti,w=new A.bU(w.a(),v.h("bU<1>")),v=v.c;w.t();){u=w.b
if(u==null)u=v.a(u)
t=s.eK(u)
if(typeof t=="string")x.i(0,t,u)}s.e!==$&&A.d1()
s.e=x
r=x}return r},
eK(d){var x,w,v,u,t=d instanceof $.ia()
if(!t)return null
$label0$0:{x=A.v(d.id)
t=x.length!==0
w=null
if(t){t=x
break $label0$0}v=A.v(d.tagName)
t=v
if("TITLE"!==t)t="BASE"===v
else t=!0
if(t){t="__"+A.v(d.tagName)
break $label0$0}if("META"===v){t=y.m
u=y.z.a(t.a(d.attributes).getNamedItem("name"))
$label1$1:{if(t.b(u)){t="__meta:"+A.v(u.value)
break $label1$1}t=w
break $label1$1}break $label0$0}t=w
break $label0$0}return t},
ma(d){var x,w,v,u,t,s,r,q,p,o,n,m,l,k,j=this
if(d||j.r){C.b.cw(j.f,new B.oZ())
j.r=!1}x=y.m
w=D.z4(j.glG(),y.N,x)
v=j.glG()
u=A.x(new A.bi(v,A.k(v).h("bi<2>")),x)
for(v=j.f,t=v.length,s=0;s<v.length;v.length===t||(0,A.ad)(v),++s)for(r=v[s].e,q=r.length,p=0;p<r.length;r.length===q||(0,A.ad)(r),++p){o=r[p]
n=j.eK(o)
if(n!=null){m=w.k(0,n)
w.i(0,n,o)
if(m!=null){C.b.i(u,C.b.cI(u,m),o)
continue}}C.b.A(u,o)}v=y.z
l=v.a(j.gex().a.nextSibling)
for(t=u.length,s=0;s<u.length;u.length===t||(0,A.ad)(u),++s){o=u[s]
if(l==null||l===j.gex().b)x.a(j.geE().insertBefore(o,l))
else if(l===o)l=v.a(l.nextSibling)
else if(j.eK(o)!=null&&j.eK(o)==j.eK(l)){r=v.a(l.parentNode)
if(r!=null)x.a(r.replaceChild(o,l))
l=v.a(o.nextSibling)}else x.a(j.geE().insertBefore(o,l))}while(!0){if(!(l!=null&&l!==j.gex().b))break
k=v.a(l.nextSibling)
t=v.a(l.parentNode)
if(t!=null)x.a(t.removeChild(l))
l=k}},
f1(){return this.ma(!1)}}
var z=a.updateTypes(["c7()","j(d4,d4)"])
B.vd.prototype={
$0(){return new N.c7()},
$S:z+0}
B.pu.prototype={
$1(d){return new A.X(this.mi(d),y.d)},
mi(d){var x=this
return function(){var w=d
var v=0,u=2,t=[],s,r,q,p,o,n,m,l,k,j,i
return function $async$$1(e,f,g){if(f===1){t.push(g)
v=u}while(true)switch(v){case 0:k=$.BB()
j=k.giO()
i=K.jz(w,!1)
i=K.bh.prototype.gP.call(i)
s=x.a
r=s.c
y.E.a(j).bP(i.w).eH(r)
q=K.rG(w,k,y.l)
$label0$0:{if(q instanceof H.b4){p=q.a
k=p
break $label0$0}k=null
break $label0$0}if(k==null){v=1
break}o=k.a
v=o==null?3:4
break
case 3:v=5
return e.b=G.af(A.b([E.et(F.fD,null),E.c5(F.e0,null)],y.i),null,"error-div",null,null,null,null),1
case 5:v=1
break
case 4:k=o.a
v=6
return e.b=new B.lX("Brainiacs.in | "+k,null),1
case 6:j=y.i
k=E.et(A.b([new D.L(k,null)],j),null)
i=y.N
n=A.bj(["popovertarget","share-overlay"],i,i)
n=H.dj(A.b([new I.aE(!1,"/images/share.svg",null,null)],j),n,"transparent-btn",null,new B.ps(s),null)
m=A.bj(["popover",""],i,i)
l=D.cY(null,null,null,null,null,null,null,null,null,null,null,A.bj(["float","right"],i,i),null,null,null,null)
i=A.bj(["popovertarget","share-overlay"],i,i)
l=H.dj(A.b([new I.aE(!1,"/images/x.svg",null,null)],j),i,"transparent-btn",null,null,l)
i=E.c5(A.b([new H.f9("Link has been copied to clipboard.<br>Share it with your friends!",null)],j),null)
r.toString
v=7
return e.b=G.af(A.b([k,n,G.af(A.b([l,i,G.af(A.b([E.b3(A.b([new D.L("https://brainiacs.in/crew?id="+r,null)],j),null,null,null),G.af(A.b([],j),null,null,null,null,null,null),H.dj(A.b([new I.aE(!1,"/images/copy.svg",null,null)],j),null,"transparent-btn",null,new B.pt(s),null)],j),null,null,null,"share-link-container",null,null)],j),m,null,null,"share-overlay",null,null)],j),null,null,null,null,null,null),1
case 7:v=8
return e.b=E.c5(A.b([new D.L(o.b,null)],j),null),1
case 8:v=9
return e.b=s.o6(o.c),1
case 9:case 1:return 0
case 2:return e.c=t.at(-1),3}}}},
$S:12}
B.ps.prototype={
$0(){var x=this.a.c
x.toString
M.xH("https://brainiacs.in/crew?id="+x)},
$S:0}
B.pt.prototype={
$0(){var x=this.a.c
x.toString
M.xH("https://brainiacs.in/crew?id="+x)},
$S:0}
B.oY.prototype={
$0(){var x,w,v,u,t,s=b.G,r=y.m,q=this.a,p=r.a(r.a(s.document).createNodeIterator(q.geE(),128))
for(x=y.z,w=null,v=null;u=x.a(p.nextNode()),u!=null;){t=A.bE(u.nodeValue)
if(t==null)t=""
if(t==="$")w=u
else if(t==="/")v=u}if(w==null){w=r.a(new s.Comment("$"))
r.a(q.geE().insertBefore(w,v))}if(v==null){v=r.a(new s.Comment("/"))
r.a(q.geE().insertBefore(v,x.a(w.nextSibling)))}return new A.hU(w,v)},
$S:64}
B.oZ.prototype={
$2(d,e){var x=y.j
x.a(d)
x.a(e)
return d.w-e.w},
$S:z+1};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(D.as,[B.mA,B.lE,B.lX])
x(A.c8,[B.vd,B.ps,B.pt,B.oY])
w(B.pu,A.az)
w(B.ln,A.dS)
w(B.ii,A.bR)
w(B.nQ,A.f6)
w(B.d4,A.cn)
w(B.lm,A.m)
w(B.oZ,A.dm)})()
A.bV(b.typeUniverse,JSON.parse('{"mA":{"as":[],"B":[]},"lE":{"as":[],"B":[]},"d4":{"cn":[],"hy":[]},"lX":{"as":[],"B":[]},"ii":{"bR":[],"B":[]},"nQ":{"bC":[],"E":[],"bg":[]}}'))
var y=(function rtii(){var x=A.u
return{l:x("a3<bq>"),U:x("ii"),j:x("d4"),h:x("E"),A:x("o<d4>"),i:x("o<B>"),O:x("o<D>"),m:x("D"),B:x("n<bA>"),E:x("aG<c7>"),N:x("i"),d:x("X<B>"),F:x("X<D>"),z:x("D?")}})();(function constants(){var x=a.makeConstList
F.cA=new B.ln(2,"head")
F.kU=new H.f9("Crew not found :/<br>Either it has been deleted or the ID (in query param) is incorrect.",null)
F.e0=A.b(x([F.kU]),y.i)
F.lg=new D.L("That's a 404!",null)
F.fD=A.b(x([F.lg]),y.i)})();(function staticFields(){$.yl=A.F(A.u("ln"),A.u("lm"))})();(function lazyInitializers(){var x=a.lazyFinal
x($,"Jv","BB",()=>H.wt(new B.vd(),A.u("c7"),A.u("bq")))})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.clients.dart.js_21",e:"endPart",h:b})})($__dart_deferred_initializers__,"ZueSilkFndhrmspDty1j6s1cF7M=");
//# sourceMappingURL=main.clients.dart.js_21.part.js.map
